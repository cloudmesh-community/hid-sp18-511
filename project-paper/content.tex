...
provides option to deploy Apache Spark Cluster and terminate the Apache
Spark Cluster once data
...
AWS~\cite{hid-sp18-511-www-aws} (see Section~\ref{S:aws}), 
Apache Spark~\cite{hid-sp18-511-www-spark}(see Section~\ref{S:spark}),
and
...
A ~\emph{Security Group} restricts the inbound and outbound traffic for
...
A \emph{Key Pair} is used to connect EC2~\cite{hid-sp18-511-www-ec2} instance
...
This section describes the setup(see Section~\ref{s:setup}) required for the project, steps for the deployment(see Section~\ref{s:cluster-deployment}) of Apache Spark~\cite{hid-sp18-511-www-spark} Cluster on EC2~\cite{hid-sp18-511-www-ec2} instances, explanation of data processing(see Section~\ref{s:steps-data-process}) tasks that can be done on Apache Spark~\cite{hid-sp18-511-www-spark} Cluster and termination steps(see Section~\ref{s:cluster-terminate}) of EC2~\cite{hid-sp18-511-www-ec2} instances once the data processing tasks complete.

\subsection{Setup}\label{s:setup}

This section describes the setup steps for Ansible(see Section~\ref{s:ansible-setup}), AWS Key(see Section~\ref{s:aws-key-setup}), Git Repository(see Section~\ref{s:git-repo-setup}) and various configuration options(see Section~\ref{s:config-opt-setup}) that
can be updated as per the project requirements.


\paragraph{Ansible}\label{s:ansible-setup}
...
\paragraph{AWS key}\label{s:aws-key-setup}
...
\paragraph{Git repository}\label{s:git-repo-setup}
...
\paragraph{Configuration options}\label{s:config-opt-setup}
...
\subsection{Data processing on Apache Spark Cluster}\label{s:steps-data-process}
...
created and is made available at \verb|Video_Part_1|~\cite{hid-sp18-511-www-video-part-1} 
...
Apache Spark~\cite{hid-sp18-511-www-spark} cluster deployment on EC2~\cite{hid-sp18-511-www-ec2} instances and termination.

\section{Results}

Table~\ref{t:results-table} provides the benchmark results.

This table provide the duration for deployment and termination of 
Spark~\cite{hid-sp18-511-www-spark} Cluster deployment on EC2 instances.
...
\begin{acks}
  ...
  support and suggestions to write this paper ans providing the technical assistance.
