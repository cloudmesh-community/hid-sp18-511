...
of machines one the data processing related task complete. User can connect to Apache
...
\begin{verbatim}
/opt/spark/spark-2.3.0-bin-hadoop2.7/examples/src/main/python/streaming/network_wordcount.py
\end{verbatim}
...
sudo bin/spark-submit --master spark://172.31.27.119:7077 examples/src/main/python/streaming/network_wordcount.py localhost 9999
\end{verbatim}
...
Once the program is executed, it will wait for the data on TCP server on localhost at port 1000.
...
\begin{figure}[!ht]
	...
	\caption{Security group}\label{f:python-spark-stream-run-command}
\end{figure}
...
\begin{figure}[!ht]
	...
	\caption{Security group}\label{f:python-stream-tcp-server.png}
\end{figure}
...
\begin{figure}[!ht]
	...
	\caption{Security group}\label{f:python_stream_prog_run_output.png}
\end{figure}
